...
const SubscriptionCard = ({
  ...
  return (
    <div
      className={`col-span-12 flex flex-col justify-between sm:col-span-6 lg:col-span-4 p-6 rounded-2xl border ${
        activePlan
          ? "border-blue-400 bg-primarySolid text-white"
          : "border-gray-400 bg-[#01183A] text-white"
      } shadow-lg `}
    >
      <div className="">
        ...
        {activePlan && timeLeft && !/NaN/.test(timeLeft) ? (
          ...
          <p className="text-sm  text-white  rounded inline-block mb-1"></p>
        )}
        ...
        <ul
          className={`space-y-2 ${
            cancelMessage || errorMessage ? "mb-2" : "mb-6"
          }`}
        >
      ...
      <>
        ...
        {cancelMessage && (
          <p className="text-sm text-center font-semibold text-green-600 mb-2">
            {cancelMessage}
          </p>
        )}
        {!activePlan || !isActive ? (
          <ActionButton
            disabled={isTrailPlan && email}
