...
export const UpcomingGames = ({ selectedGame }) => {
  ...
  const { upcomingFixtures, fixturesLoading } = useSelector(
    (state) => state.fixtures
  );
  const { AFfixturesLoading, AFupcomingFixtures } = useSelector(
    (state) => state.fixtures
  );
  console.log("AFupcomingFixtures", AFupcomingFixtures);
    ...
    ? [
        ...new Map(
          AFupcomingFixtures.data
            .flatMap((league) =>
              Array.isArray(league.upcomingGames) ? league.upcomingGames : []
    ...
    <section className="w-full h-full max-md:max-w-full">
      <div className="flex justify-between">
        <h2 className="text-2xl flex items-center font-bold leading-none max-md:max-w-full text-white">
          Upcoming {isSoccer ? "Soccer" : "Football"} Games
        </h2>
      ...
      <div className="mt-6 w-full text-lg text-center max-md:max-w-full">
        ...
        paginatedGames.length === 0 ? (
          ...
          paginatedGames.map((game, index) => (
            ...
            >
              ...
              <div className="flex items-center justify-center gap-4 w-full max-md:flex-col">
                ...
                <div className="text-sm font-semibold w-1/3 text-center max-md:w-full">
                  VS
                </div>
                <div className="flex items-center justify-center md:justify-start gap-2 w-1/3 text-center max-md:w-full">
                  <img
          ...
          <EmptyState
            icon={FaRegSadTear}
            message="No upcoming matches to show"
          />
        )}
