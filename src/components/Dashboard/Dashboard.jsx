...
const Dashboard = () => {
  ...
  const { playerOrTeam, sportsNameDashboard } = useSelector(
    (state) => state.selection
  ); // Access Redux state
  const [sportsName, setSportsName] = useState("soccer");
  const { favoritePlayers, favoriteTeams } = useSelector(
    (state) => state.favorites
  ); // Get from Redux
  ...
  ]); // Re-run only if sportsNameDashboard changes OR leagues data is empty
      ...
      <main className="flex grid grid-cols-12 overflow-hidden min-h-screen bg-primary  px-5 sm:px-8 py-6 flex-col">
        ...
        {/* <div className="col-span-12 md:col-span-6 mb-4 flex gap-2">
                ...
                ${sportsNameDashboard === sport ? "bg-primarySolid" : "bg-searchBarBg"}`}
                        >
                            {sport === "soccer" ? "Soccer" : sport === "basketball" ? "Basketball" : "Football"}
                        </button>
                    ))}
                </div> */}
        <div className="col-span-12 md:col-span-6 mb-4 flex gap-2">
          {["soccer", "american-football"].map((sport) => {
            const isSelected = sportsNameDashboard === sport;
            const isDisabled = isSelected || favoritePlayersHighlightsLoading;

            return (
              <button
                key={sport}
                onClick={() => {
                  if (!isSelected && !favoritePlayersHighlightsLoading) {
                    dispatch(setSportsNameDashboard(sport));
                    dispatch(resetHighlights());
                  }
                }}
                ...
                className={`px-4 py-2 rounded-full whitespace-nowrap w-full text-white transition-all 
                    ${
                      isSelected
                        ? "bg-primarySolid"
                        : "bg-searchBarBg hover:opacity-100"
                    } 
                    ${
                      isDisabled
        ...
        <div className="col-span-12 md:col-span-8 w-full md:w-auto mr-4">
          ...
          <div className=" flex ">
            ...
            <div className="relative lg:w-1/6">
              ...
              >
                <option className="text-black" value="player">
                  Player
                </option>
              ...
              <section className="mt-6 w-full">
                <div className="grid grid-cols-1  gap-4">
                  ...
                  <div className="md:col-span-2 ">
                    <LeagueAreaChart
                      gameType={sportsNameDashboard}
                      data={favoriteLeaguessHighlights}
                    />
                  </div>
                  {/* <AlertCard /> */}
                </div>
