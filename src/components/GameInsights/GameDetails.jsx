...
import {
  fetchUpcomingAFFixturesPredictions,
  fetchUpcomingFixturesPredictions,
...
const GameDetails = () => {
  ...
  const upcomingFixturesPredictions = useSelector(
    (state) => state.prediction.upcomingFixturesPredictions
  );
  const AFupcomingFixturesPredictions = useSelector(
    (state) => state.prediction.AFupcomingFixturesPredictions
  );
  ...
  if (gameType === "soccer") {
    if (status === "live") {
      game = liveFixtures?.data?.find(
        (fixture) => fixture.fixture.id.toString() === fixtureId
      );
      ...
      if (fixtureList) {
        ...
        game = league?.fixtures?.find(
          (fixture) => fixture.fixtureId.toString() === fixtureId
        );
      }
    }
  }
      ...
      fixtureList?.data?.forEach((leagueItem) => {
        const foundGame = (
          status === "live"
            ...
            : status === "completed"
            ? leagueItem?.completedGames
            : leagueItem?.upcomingGames
        )?.find(
          (gameItem) =>
            gameItem.league.leagueId.toString() === leagueId &&
            gameItem.gameId.toString() === fixtureId
        );
  ...
  if (fixturesLoading || AFfixturesLoading || predictionLoading)
    return (
  ...
  if (!game)
    return (
      <div className="bg-primary min-h-screen text-white p-6 text-center">
        Game not found
      </div>
    );
  ...
  return (
    <div className="bg-primary min-h-screen text-white px-5 py-6 sm:p-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-300">
        {league?.country && (
          <img
            src={league.country.flag || game.league.flag}
            alt={league.country.name || league}
            className="w-5 h-5"
          />
        )}
      ...
      <div className="flex flex-col mb-6 items-center justify-center mt-6">
        <p className="text-base font-semibold">
          {formatDateTime(
            gameType === "soccer"
              ? game.date
              : game.date?.date || game.fixture?.date
          ) || ""}
        </p>
        {game?.venue && (
        ...
        {status === "live" && (
          <p className="text-base font-semibold">
            {gameType === "soccer" && game.fixture.status.elapsed + " min"}
          </p>
        )}
      ...
      <div className="flex border-gray-500 pb-1 justify-start gap-3 my-4">
        {(status === "completed" || status === "live") && (
          <NavLink
            to="summary"
            className={({ isActive }) =>
              `px-1 pb-2 text-base text-white ${
                isActive
