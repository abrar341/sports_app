...
export const CompletedGames = ({ selectedGame }) => {
  ...
  //soccer
  const { completedFixtures, fixturesLoading } = useSelector(
    (state) => state.fixtures
  ); // Soccer
  //american-football
  const { AFcompletedFixtures, AFfixturesLoading } = useSelector(
    (state) => state.fixtures
  ); // American-Football
  ...
  useEffect(() => {
    if (fixturesData?.data?.length > 0) {
      setSelectedLeague(
        isSoccer
          ? fixturesData.data[0].leagueId
          : fixturesData.data[0].leagueName
      );
    }
  ...
  const selectedFixtures = fixturesData?.data?.find((league) =>
    isSoccer
      ? Number(league.leagueId) === Number(selectedLeague)
      : league.leagueName === selectedLeague
  );
  ...
  const handleGameClick = (leagueId, fixtureId, status) => {
    navigate(
      `/games-insight/${selectedGame}/${leagueId}/${status}/${fixtureId}/summary`
    );
  };
    ...
    <section className="mt-6 h-full w-full min-h-[270px] max-md:max-w-full">
      <div className="flex justify-between">
        <h2 className="text-2xl flex items-center font-bold leading-none max-md:max-w-full text-white">
          Completed {isSoccer ? "Soccer" : "Football"} Games
        </h2>
      ...
      <div className="mt-6 w-full text-lg text-center max-md:max-w-full">
        ...
        (!completedFixtures.data || !AFcompletedFixtures.data) ? (
          <Loading />
          ...
          paginatedGames.map((game, index) => (
            <div
              onClick={() =>
                handleGameClick(
                  isSoccer ? selectedLeague : game.league.leagueId,
                  game.gameId || game.fixtureId,
                  "completed"
                )
              }
              key={index}
          ...
          <EmptyState
            icon={FaRegSadTear}
            message="No completed matches to show"
          />
        )}
