...
const PlayerStatsWidget = ({ gameType }) => {
  ...
  const { favoritePlayersHighlightsLoading } = useSelector(
    (state) => state.loading
  );
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fade, setFade] = useState(true);
  const [hovered, setHovered] = useState(false);
  const { handleSelectPlayer, handleSelectAmericanFootballPlayer } =
    usePlayerActions();

  useEffect(() => {
    if (
      !favoritePlayersHighlights ||
      favoritePlayersHighlights.length <= 1 ||
      hovered
    )
      return;
  ...
  if (
    !favoritePlayersHighlights ||
    (favoritePlayersHighlights.length === 0 &&
      !favoritePlayersHighlightsLoading)
  )
    return (
      <div className="rounded-xl bg-secondary h-full">
        <EmptyState
          icon={FaRegSadTear}
          message="No favorite players added yet."
        />
      </div>
    );
  ...
  const getAmericanFootballStats = (category, statName) => {
    const categoryData = currentPlayer?.statsSummary?.statistics?.find(
  ...
  const statsData =
    gameType === "soccer"
      ? [
          {
            label: "Goals",
            value: currentPlayer?.statsSummary?.totalGoals ?? "-",
          },
          {
            label: "Minutes",
            value: currentPlayer?.statsSummary?.totalMinutesPlayed ?? "-",
          },
          {
            label: "Matches",
            value: currentPlayer?.statsSummary?.totalMatchesPlayed ?? "-",
          },
        ]
      : [
          {
            label: "Rush Yards",
            value: getAmericanFootballStats("Rushing", "yards"),
          },
          {
            label: "Rec Yards",
            value: getAmericanFootballStats("Receiving", "receiving yards"),
          },
          {
            label: "Total TDs",
            value: getAmericanFootballStats("Scoring", "total touchdowns"),
          },
        ];
